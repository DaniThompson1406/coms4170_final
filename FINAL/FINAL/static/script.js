...
function initDragDrop() {
    console.log("initDragDrop running");
    ...
    milkTarget.addEventListener('drop', function (e) {
        ...
        if (draggedName === correctDrag) {
            // Correct - Remove all items and show drink
            document.querySelector('.quiz-container').innerHTML = `
                <div style="text-align:center;">
                    <img src="${document.querySelector('body').dataset.correctimg}" alt="${correctDrag}" style="width:200px;">
                </div>
            `;
            document.getElementById('form-section').style.display = 'block';
        } else {
            // Incorrect - Deduct point only
